...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def increment(acc: List[(Char, Int)], char: Char): List[(Char, Int)] = {
      acc match {
        case Nil => (char, 1) :: Nil
        case (c, i) :: rest => {
          if (char == c) (char, i + 1) :: rest
          else (c, i) :: increment(rest, char)
        }
      }
    }
    ...
    def loop(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = {
      chars match {
        case Nil => acc
        case char :: rest => {
          val updatedAcc = increment(acc, char)
          loop(rest, updatedAcc)
        }
      }
    }
  ...
  def insert[T](value: T, list: List[(T)], comparator: (x: T, y: T) => Boolean): List[(T)] = {
    list match {
      case Nil => value :: Nil
      case headValue :: rest => {
        if (comparator(value, headValue)) value :: headValue :: rest
        else headValue :: insert(value, rest, comparator)
      }
    }
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    // insert sort
    // merge sort possible but a bit difficult with pure recursion
    // in particular finding the middle of the linked list is a pain without a while loop
    def loop(
        freqs: List[(Char, Int)],
        acc: List[(Leaf)]
    ): List[(Leaf)] = {
      freqs match {
        case Nil => acc
        case (char, int) :: rest => {
          val leaf = Leaf(char, int)
          val updatedAcc = insert[Leaf](leaf, acc, (x: Leaf,y: Leaf) => x.weight < y.weight)
          loop(rest, updatedAcc)
        }
      }
    }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
      ...
      case _ => {
        val el1 = trees.head
        val el2 = trees.tail.head
        val newCodeTree = makeCodeTree(el1, el2)
  ...
  def decodeOneChar(tree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = {
    tree match {
      ...
      case f: Fork => {
        bits.head match {
          case 0 => decodeOneChar(f.left, bits.tail)
          case 1 => decodeOneChar(f.right, bits.tail)
        }
      }
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    bits match {
      case Nil => Nil
      case _ => {
        val (char, newBits) = decodeOneChar(tree, bits)
  ...
  /** This function encodes `text` using the code tree `tree` into a sequence of
    * bits.
    */
  def encodeOneChar(tree: CodeTree, c: Char): Option[List[Bit]] = {
    tree match {
      case l: Leaf => {
        if(l.char == c) Some(Nil)
        else None
      }
      case f: Fork => {
        val leftOpt = encodeOneChar(f.left, c)
        val rightOpt = encodeOneChar(f.right, c)
        if(leftOpt.isDefined) Some(0 :: leftOpt.get) 
        else if(rightOpt.isDefined) Some(1 :: rightOpt.get)
        else None
      }
    }
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    text match {
      case Nil => Nil
      case c :: rest => encodeOneChar(tree, c).get ::: encode(tree)(rest)
    }
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table match {
      case Nil => Nil
      case (bitListChar, bitList) :: rest => {
        if(bitListChar == char) bitList
  ...
  def addBit(table: CodeTable, bit: Bit): CodeTable = {
    table match {
      case Nil => Nil
      case (c: Char, bitList: List[Bit]) :: rest => 
        (c, bit :: bitList) :: addBit(rest, bit)
    }
  }

  def convert(tree: CodeTree): CodeTable = tree match {
    case Leaf(c, w) => List((c, Nil))
    case Fork(l, r, c, w) => addBit(convert(l), 0) ::: addBit(convert(r), 1)
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def loop(table: CodeTable, text: List[Char]): List[Bit] = {
      text match {
        case Nil => Nil
        case char :: rest => codeBits(table)(char) ::: loop(table, rest)
      }
    }
    loop(table, text)
  }
