...
abstract class TweetSet extends TweetSetInterface:
  ...
  def descendingByRetweet: TweetList = {
    if (isEmpty) Nil
      ...
      val greatestTweet = mostRetweeted
      Cons(greatestTweet, remove(greatestTweet).descendingByRetweet)
    }
  }
...
// would be better as an object but as an object it fails tests
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val includeLeft = left.filterAcc(p, acc)
    val includeBoth = right.filterAcc(p, includeLeft)
    if (p(elem)) includeBoth.incl(elem) else includeBoth
  }
  ...
  def mostRetweeted: Tweet = {
    def tweetWithMoreRetweets(x: Tweet, y: Tweet): Tweet =
      if (x.retweets >= y.retweets) x else y
    val greatestLeft = if (left.isEmpty) elem else left.mostRetweeted
    val greatestRight = if (right.isEmpty) elem else right.mostRetweeted
    val greatestChild =
      tweetWithMoreRetweets(greatestLeft, greatestRight)
    tweetWithMoreRetweets(elem, greatestChild)
  }
