...
object Lists:
  ...
  def max(xs: List[Int]): Int = {
    if xs.isEmpty then
      throw java.util.NoSuchElementException("no max on empty list")
    else maxList(xs: List[Int])
  }

  def maxList(xs: List[Int]): Int = {
    if (xs.isEmpty) 0 else maxInts(xs.head, maxList(xs.tail))
  }

  def maxInts(x: Int, y: Int): Int = {
    if (x > y) x else y
  }
